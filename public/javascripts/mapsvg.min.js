...
(function ($, window, MapSVG, Math) {
	...
	MapSVG.ResizeSensor.prototype.setScroll = function () {
		this.expand.scrollLeft = 1e7;
		this.expand.scrollTop = 1e7;
		this.shrink.scrollLeft = 1e7;
		this.shrink.scrollTop = 1e7
	};
	...
	MapSVG.touchDevice = MapSVG.userAgent.indexOf("ipad") > -1 || MapSVG.userAgent.indexOf("iphone") > -1 || MapSVG.userAgent.indexOf("ipod") > -1 || MapSVG.userAgent.indexOf("android") > -1;
	MapSVG.ios = MapSVG.userAgent.indexOf("ipad") > -1 || MapSVG.userAgent.indexOf("iphone") > -1 || MapSVG.userAgent.indexOf("ipod") > -1;
	...
	MapSVG.safeURL = function (url) {
		if (url.indexOf("http://") == 0 || url.indexOf("https://") == 0) url = "//" + url.split("://").pop();
		return url.replace(/^.*\/\/[^\/]+/, "")
	};
	...
	MapSVG.Controller.prototype.updateScroll = function () {
		var _this = this;
		this.contentWrap.nanoScroller({
			preventPageScrolling: true,
			iOSNativeScrolling: true
		});
		setTimeout(function () {
			_this.contentWrap.nanoScroller({
				preventPageScrolling: true,
				iOSNativeScrolling: true
			})
		}, 300)
	...
	MapSVG.Controller.prototype.render = function () {
		...
		if (this.width) this.view.css({
			width: this.width
		});
		if (this.color) this.view.css({
	...
	MapSVG.Controller.prototype.redraw = function () {
		...
		this.updateTopShift();
		if (this.noPadding) this.contentView.css({
			padding: 0
		});
		if (this.scrollable) this.updateScroll()
	};
	MapSVG.Controller.prototype.updateTopShift = function () {
		var _this = this;
		_this.contentWrap.css({
			top: _this.toolbarView.outerHeight(true) + "px"
		});
		setTimeout(function () {
			_this.contentWrap.css({
				top: _this.toolbarView.outerHeight(true) + "px"
	...
	MapSVG.DirectoryController.prototype.viewDidLoad = function () {
		...
		if (this.mapsvg.getData().options.menu.on !== false) this.mobileButtons.insertAfter(this.mapsvg.getData().$wrap);
		...
		if (this.mapsvg.getData().options.colors.directorySearch) {
			this.toolbarView.css({
				"background-color": this.mapsvg.getData().options.colors.directorySearch
			})
		}
	};
	MapSVG.DirectoryController.prototype.setEventHandlers = function () {
		...
		this.view.on("keyup.menu.mapsvg", ".mapsvg-directory-search", function () {
			...
			if (_this.mapsvg.getData().options.menu.searchFallback) data.searchFallback = true;
		...
		this.view.on("click.menu.mapsvg", ".mapsvg-directory-item", function (e) {
			...
			if (detailsViewObject.marker && detailsViewObject.marker.id) marker = _this.mapsvg.getMarker(detailsViewObject.marker.id);
			...
			if (regions && regions.length > 0) {
				...
				regions.forEach(function (region) {
					...
					if (_this.mapsvg.getData().options.actions.directoryItem.click.fireRegionOnClick) {
						...
						if (events && events["click.region"]) events && events["click.region"].call(region, e, _this.mapsvg)
					}
				});
				if (regions.length > 1) {
			...
			if (marker) {
				...
				if (_this.mapsvg.getData().options.actions.directoryItem.click.fireMarkerOnClick) {
					var events = _this.mapsvg.getData().events;
					if (events && events["click.marker"]) events && events["click.marker"].call(marker, e, _this.mapsvg)
				}
			}
	...
	MapSVG.DetailsController.prototype.getToolbarTemplate = function () {
		if (this.withToolbar) return '<div class="mapsvg-popover-close mapsvg-details-close"></div>';
		else return ""
	};
	...
	MapSVG.PopoverController.prototype.setEventHandlers = function () {
		...
		$("body").one("mouseup.popover.mapsvg touchend.popover.mapsvg ", function (e) {
			setTimeout(function () {
				if (_this.mapsvg.getData().isScrolling || $(e.target).closest(".mapsvg-popover").length || $(e.target).hasClass("mapsvg-btn-zoom")) return;
				_this.close()
			}, 50)
		})
	};
	MapSVG.PopoverController.prototype.close = function () {
		var _this = this;
		if (this.container.data("popover-id") != this.id || !_this.container.is(":visible")) return;
		_this.destroy();
...
(function ($) {
	...
	MapObject.prototype.getBBox = function () {
		...
		var bbox = this instanceof Marker ? {
			x: this.x,
			y: this.y,
			width: this.default.width / _data.scale,
			height: this.default.height / _data.scale
		} : this.node[0].getBBox();
	...
	MapObject.prototype.getComputedStyle = function (prop, node) {
		...
		if (nodeType && nodeType != "svg") return this.getComputedStyle(prop, parent[0]);
	...
	MapObject.prototype.setHref = function (url) {
		if (this.href && this.node.parent("a").length) this.node.unwrap();
	...
	Region.prototype.setFill = function (fill) {
		...
		if (fill) {
			...
			$.extend(true, this.globalOptions, {
				regions: regions
			})
		...
		if (MapSVG.isNumber(this.globalOptions.colors.selected)) this.selected_attr["fill"] = MapSVG.tinycolor(this.default_attr.fill).lighten(parseFloat(this.globalOptions.colors.selected)).toRgbString();
		else this.selected_attr["fill"] = this.globalOptions.colors.selected;
		if (MapSVG.isNumber(this.globalOptions.colors.hover)) this.hover_attr["fill"] = MapSVG.tinycolor(this.default_attr.fill).lighten(parseFloat(this.globalOptions.colors.hover)).toRgbString();
	...
	Region.prototype.getDisabledState = function (asDefault) {
		...
		} else if (this.globalOptions.disableAll || this.style.fill == "none" || this.id == "labels" || this.id == "Labels") {
			return true
		} else {
			return false
		}
	...
	Region.prototype.highlight = function () {
		this.node.css({
			fill: this.hover_attr.fill
		})
	};
	Region.prototype.unhighlight = function () {
		this.node.css({
			fill: this.default_attr.fill
		})
	};
	Region.prototype.select = function () {
		this.node.css({
			fill: this.selected_attr.fill
		});
		this.selected = true
	};
	...
	Marker.prototype.setXy = function (xy) {
		...
		this.y = xy[1];
		this.xy = [this.x, this.y];
		if (this.mapsvg.getData().mapIsGeo) this.geoCoords = this.mapsvg.convertSVGToGeo(xy[0], xy[1]);
		this.adjustPosition();
	...
	Marker.prototype.moveToClick = function (xy) {
		...
		if (_data.mapIsGeo) this.geoCoords = this.mapsvg.convertSVGToGeo(xy[0], xy[1]);
		markerOptions.xy = xy;
	...
	var markerOptions = {
		src: pluginRootURL + "markers/pin1_red.png"
	};
	mapSVG = function (elem, options) {
		...
		this.methods = {
			...
			setLayersControl: function (options) {
				...
				if (_data.options.layersControl.on) {
					...
					_data.options.groups.forEach(function (g) {
						var item = $('<div class="mapsvg-layers-item" data-group-id="' + g.id + '">' + '<input type="checkbox" class="ios8-switch ios8-switch-sm" ' + (g.visible ? "checked" : "") + " />" + "<label>" + g.title + "</label> " + "</div>").appendTo(_data.$layersControlList)
					});
			...
			setFloorsControl: function (options) {
				...
				if (_data.options.floorsControl.on) {
					...
					_data.options.floors.forEach(function (f) {
						var item = $('<div class="mapsvg-floors-item" data-floor-id="' + f.object_id + '">' + "<label>" + f.title + "</label> " + "</div>").appendTo(_data.$floorsControlList)
					});
			...
			getPagination: function (callback) {
				...
				_data.pager = $('<nav class="mapsvg-pagination"><ul class="pager"><!--<li class="mapsvg-first"><a href="#">First</a></li>--><li class="mapsvg-prev"><a href="#">← Prev. ' + _data.options.database.pagination.perpage + '</a></li><li class="mapsvg-next"><a href="#">Next ' + _data.options.database.pagination.perpage + ' →</a></li><!--<li class="mapsvg-last"><a href="#">Last</a></li>--></ul></nav>');
				if (_this.database.onFirstPage() && _this.database.onLastPage()) {
					...
					_this.database.onLastPage() && (_data.pager.find(".mapsvg-next").addClass("disabled") && _data.pager.find(".mapsvg-last").addClass("disabled"));
					_this.database.onFirstPage() && (_data.pager.find(".mapsvg-prev").addClass("disabled") && _data.pager.find(".mapsvg-first").addClass("disabled"))
				}
				_data.pager.on("click", ".mapsvg-next:not(.disabled)", function (e) {
					e.preventDefault();
					if (_this.database.onLastPage()) return;
					_this.database.getAll({
						page: _this.database.page + 1
					}).done(function () {
						callback && callback()
					})
			...
			getOptions: function (forTemplate, forWeb, optionsDelta) {
				...
				if (forWeb) $.each(options, function (key, val) {
					if (JSON.stringify(val) == JSON.stringify(defaults[key])) delete options[key]
				});
				return options
			},
			...
			setEvents: function (functions) {
				...
				for (var i in functions) {
					if (typeof functions[i] === "string") {
						...
						if (func && !func.error && !(func instanceof TypeError || func instanceof SyntaxError)) _data.events[i] = func;
			...
			setColors: function (colors) {
				...
				if (colors.background) _data.$map.css({
					background: _data.options.colors.background
				});
				if (colors.hover) _data.options.colors.hover = colors.hover == "" + parseInt(colors.hover) ? parseInt(colors.hover) : colors.hover;
				if (colors.selected) _data.options.colors.selected = colors.selected == "" + parseInt(colors.selected) ? parseInt(colors.selected) : colors.selected;
				_this.regionsRedrawColors()
			},
				...
				if (_data.options.zoom.on && _data.options.zoom.mousewheel) {
					if (MapSVG.browser.firefox) {
						...
						$(document).scroll(function () {
							if (_data.firefoxScroll.insideIframe) window.scrollTo(_data.firefoxScroll.scrollX, _data.firefoxScroll.scrollY)
						})
					}
					_data.$map.on("mousewheel.mapsvg", function (event, delta, deltaX, deltaY) {
						if ($(event.target).hasClass("mapsvg-popover") || $(event.target).closest(".mapsvg-popover").length) return;
			...
			setZoomButtons: function () {
				...
				loc == "right" && _data.zoomButtons.addClass("right") || loc == "left" && _data.zoomButtons.addClass("left");
				...
				if (_data.options.menu.on) {
					...
					if (menuContainer.length) {
						if (!_data.$menu) {
							...
							if (!_data.$menu.hasClass("mapsvg-menu")) _data.$menu.addClass("mapsvg-menu")
						}
						if (_data.$menu.children().length === 0) _data.regions.forEach(function (region, i) {
							if (!region.disabled) _data.$menu.append(_data.options.menu.template(region))
						});
					...
					if (!MapSVG.googleMapsApiLoaded) {
						...
						if (!_data.googleMaps.map) {
							...
							USGSOverlay.prototype.draw = function () {
								...
								var vb = [_data.svgDefault.viewBox[0] - sw2.x / scale, _data.svgDefault.viewBox[1] - ne2.y / scale, _data.$map.width() / scale, _data.$map.outerHeight() / scale];
								_this.setViewBox(vb)
							};
			...
			loadGoogleMapsAPI: function (callback, fail) {
				...
				_data.googleMapsScript.src = "//maps.googleapis.com/maps/api/js?key=" + _data.options.googleMaps.apiKey;
				...
				if (_data.options.menuMarkers.on) {
					...
					if (menuContainer.length) {
						if (!_data.$menuMarkers) {
							...
							if (!_data.$menuMarkers.hasClass("mapsvg-menu-markers")) _data.$menuMarkers.addClass("mapsvg-menu-markers")
						}
						if (_data.$menuMarkers.children().length === 0) _data.markers.sort(function (a, b) {
			...
			checkId: function (id) {
				if (_this.getRegion(id)) return {
					error: "This ID is already being used by a Region"
				};
				else if (_this.getMarker(id)) return {
			...
			updateSize: function () {
				...
				if (_data.directoryWrap) _data.directoryWrap.height(_data.$wrap.outerHeight())
			},
			getViewBox: function () {
			...
			zoomTo: function (region, zoomToLevel) {
				...
				if (_data.googleMaps.map) {
					if (region instanceof Marker) {
						...
						_data.googleMaps.map.setCenter({
							lat: latlng[0],
							lng: latlng[1]
						})
					} else {
				...
				if (region instanceof Marker) {
					...
					_this.setViewBox([region.x - vb[2] / 2, region.y - vb[3] / 2, vb[2], vb[3]]);
				...
				$.each(_data.zoomLevels, function (key, level) {
					if (viewBoxPrev.length) {
						if (bbox[2] > bbox[3] && (viewBoxPrev[2] > bbox[2] && bbox[2] > level.viewBox[2])) {
							...
							var vb = _data.zoomLevels[_data.zoomLevel].viewBox;
							_this.setViewBox([bbox[0] - vb[2] / 2 + bbox[2] / 2, bbox[1] - vb[3] / 2 + bbox[3] / 2, vb[2], vb[3]]);
							_data._scale = _data.zoomLevels[_data.zoomLevel]._scale
			...
			centerOn: function (region, yShift) {
				if (_data.options.googleMaps.on) {
					...
					_this.setViewBox([bbox[0] - vb[2] / 2 + bbox[2] / 2, bbox[1] - vb[3] / 2 + bbox[3] / 2 - yShift, vb[2], vb[3]])
				}
			},
			zoom: function (delta, center, exact) {
				...
				if (!exact) {
					...
					_data._zoomLevel += d;
					if (isInZoomRange && (_data._zoomLevel > _data.options.zoom.limit[1] || _data._zoomLevel < _data.options.zoom.limit[0])) return false;
				...
				if (_data.options.scroll.limit) {
					if (newViewBox[0] < _data.svgDefault.viewBox[0]) newViewBox[0] = _data.svgDefault.viewBox[0];
					else if (newViewBox[0] + newViewBox[2] > _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2]) newViewBox[0] = _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2] - newViewBox[2];
					if (newViewBox[1] < _data.svgDefault.viewBox[1]) newViewBox[1] = _data.svgDefault.viewBox[1];
					else if (newViewBox[1] + newViewBox[3] > _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3]) newViewBox[1] = _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3] - newViewBox[3]
				}
				_this.setViewBox(newViewBox)
			},
			...
			scrollStart: function (e, mapsvg) {
				if ($(e.target).hasClass("mapsvg-btn-zoom") || $(e.target).closest(".mapsvg-gauge").length) return false;
				if (_data.editMarkers.on && $(e.target).hasClass("class") == "mapsvg-marker") return false;
			...
			scrollMove: function (e) {
				...
				if (_data.options.scroll.limit) {
					...
					else if (_data.viewBox[2] + vx > _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2]) vx = _data.svgDefault.viewBox[0] + _data.svgDefault.viewBox[2] - _data.viewBox[2];
					if (vy < _data.svgDefault.viewBox[1]) vy = _data.svgDefault.viewBox[1];
					else if (_data.viewBox[3] + vy > _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3]) vy = _data.svgDefault.viewBox[1] + _data.svgDefault.viewBox[3] - _data.viewBox[3]
				}
				_data.scroll.vx = vx;
				_data.scroll.vy = vy
			},
			scrollEnd: function (e, mapsvg, noClick) {
				...
				if (noClick !== true && Math.abs(_data.scroll.dx) < 5 && Math.abs(_data.scroll.dy) < 5) {
					if (_data.editMarkers.on) _data.clickAddsMarker && _this.markerAddClickHandler(e);
					else if (_data.region_clicked) _this.regionClickHandler(e, _data.region_clicked)
				}
			...
			touchStart: function (_e, mapsvg) {
				...
				if (_data.options.zoom.on && e.touches && e.touches.length == 2) {
					...
					_data.scaleDistStart = Math.hypot(e.touches[0].pageX - e.touches[1].pageX, e.touches[0].pageY - e.touches[1].pageY)
			...
			touchMove: function (_e, mapsvg) {
				...
				if (_data.options.zoom.on && e.touches && e.touches.length == 2) {
					if (!MapSVG.ios) {
						e.scale = Math.hypot(e.touches[0].pageX - e.touches[1].pageX, e.touches[0].pageY - e.touches[1].pageY) / _data.scaleDistStart
					}
			...
			pickGaugeColor: function (gaugeValue) {
				...
				var rgb = [Math.round(_data.options.gauge.colors.diffRGB.r * w + _data.options.gauge.colors.lowRGB.r), Math.round(_data.options.gauge.colors.diffRGB.g * w + _data.options.gauge.colors.lowRGB.g), Math.round(_data.options.gauge.colors.diffRGB.b * w + _data.options.gauge.colors.lowRGB.b), Math.round(_data.options.gauge.colors.diffRGB.a * w + _data.options.gauge.colors.lowRGB.a)];
				return rgb
			},
			isRegionDisabled: function (id, svgfill) {
				if (_data.options.regions[id] && (_data.options.regions[id].disabled || svgfill == "none")) {
					return true
				} else if ((_data.options.regions[id] == undefined || MapSVG.parseBoolean(_data.options.regions[id].disabled)) && (_data.options.disableAll || svgfill == "none" || id == "labels" || id == "Labels")) {
					return true
				} else {
					return false
				}
			...
			regionClickHandler: function (e, region, skipPopover) {
				...
				var ajax_params = {
                'url'     : "https://user.tjhsst.edu/2021jpesaven/map_worker",
                'type'    : "get",
                'data'    : {'choice' : region.id},
                'success' : (response) => {
                    leaderString = '';
			...
			search: function (str) {
				...
				str = str.toLowerCase();
				_data.regions.forEach(function (r) {
					if (r.id.toLowerCase().indexOf(str) === 0 || r.id.toLowerCase().indexOf("-" + str) !== -1 || r.title && r.title.toLowerCase().indexOf(str) === 0) results.push({
						id: r.id,
						id_no_spaces: r.id_no_spaces
					})
				});
				return results
			},
			...
			searchData: function (field, str) {
				...
				_data.options.data.forEach(function (params) {
					for (var i in params) {
						if (("" + params[i]).toLowerCase().indexOf(str) === 0 && results.indexOf(params.id) == -1) results.push(params.id)
					}
				});
				return results
			},
			...
			showTooltip: function (region) {
				...
				if (_data.options.tooltips.priority == "global") {
					tip = typeof _data.options.tooltips.mode == "function" && _data.options.tooltips.mode.call(region, _data.tooltip.container, region, _this) || _data.options.tooltips.mode != "off" && region[_data.options.tooltips.mode] || region.tooltip
				} else {
					tip = region.tooltip || typeof _data.options.tooltips.mode == "function" && _data.options.tooltips.mode.call(region, _data.tooltip.container, region, _this) || _data.options.tooltips.mode != "off" && region[_data.options.tooltips.mode]
				}
				if (tip && tip.length) {
			...
			popoverOffHandler: function (e) {
				if (_data.isScrolling || $(e.target).closest(".mapsvg-popover").length || $(e.target).hasClass("mapsvg-btn-zoom")) return;
			...
			setEventHandlers: function () {
				...
				if (_data.options.scroll.spacebar) {
					...
					if (!_data.editMarkers.on) {
						...
						if (_data.clickAddsMarker) _data.$map.on("touchend.common.mapsvg mouseup.common.mapsvg", function (e) {
							if (e.cancelable) {
								e.preventDefault()
							}
			...
			init: function (opts, elem) {
				...
				if (opts.beforeLoad) try {
					opts.beforeLoad.call(_this)
				} catch (err) {}
				if (opts.source.indexOf("//") === 0) opts.source = opts.source.replace(/^\/\/[^\/]+/, "").replace("//", "/");
				...
				_data.eventsPrevent = {};
				_data.geoCoordinates = false, _data.geoViewBox = {
					leftLon: 0,
					topLat: 0,
					rightLon: 0,
					bottomLat: 0
				...
				$.ajax({
					...
					_this.setGoogleMaps();
					if (_data.options.tooltipsMode) _data.options.tooltips.mode = _data.options.tooltipsMode;
					_this.setTooltips(_data.options.tooltips);
					if (_data.options.popover) _data.options.popovers = _data.options.popover;
...
jQuery(document).ready(() => {
        ...
        function initMap() {
            jQuery("#mapsvg").mapSvg({height: window.innerHeight - 100,colors: {baseDefault: "#000000",background: "#DADADA",directory: "#fafafa",status: {},base: "#A17917",disabled: "0000ff",selected: "#790201",hover: 20,stroke: "#000000"}, viewBox: [0,0,610,600],gauge: {on: false,labels: {low: "low",high: "high"},colors: {lowRGB: {r: 85,g: 0,b: 0,a: 1},highRGB: {r: 238,g: 0,b: 0,a: 1},low: "#550000",high: "#ee0000",diffRGB: {r: 153,g: 0,b: 0,a: 0}},min: 0,max: false},source: "media/europe.svg",title: "Europe",responsive: false});
        }